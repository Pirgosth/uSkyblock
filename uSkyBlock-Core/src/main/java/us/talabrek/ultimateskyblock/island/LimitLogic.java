...
public class LimitLogic {
	public enum CreatureType {
		UNKNOWN, ANIMAL, ACQUATIC, MONSTER, SLIME, VILLAGER, GOLEM
	}
	...
	public CreatureType getCreatureType(LivingEntity creature) {
		...
		} else if (creature instanceof Monster || creature instanceof Ghast) {
	...
	private int getMax(us.talabrek.ultimateskyblock.api.IslandInfo islandInfo, CreatureType creatureType) {
		...
		case ACQUATIC:
			return islandInfo.getMaxAcquatics();
		case MONSTER:
			return islandInfo.getMaxMonsters();
		case SLIME:
			return islandInfo.getMaxSlimes();
		case VILLAGER:
	...
	public String getSummary(us.talabrek.ultimateskyblock.api.IslandInfo islandInfo) {
		...
		for (LimitLogic.CreatureType key : creatureMax.keySet()) {
			...
			sb.append(tr("\u00a77{0}: \u00a7a{1}\u00a77 (max. {2})", tr(key.name()),
					cnt >= max ? tr("\u00a7c{0}", cnt) : cnt, max) + "\n");
		}
		...
		for (Map.Entry<Material, Integer> entry : blockLimits.entrySet()) {
			...
			if (blockCount >= 0) {
				sb.append(tr("\u00a77{0}: \u00a7a{1}\u00a77 (max. {2})",
						...
						blockCount >= entry.getValue() ? tr("\u00a7c{0}", blockCount) : blockCount, entry.getValue())
						+ "\n");
			} else {
				sb.append(tr("\u00a77{0}: \u00a7a{1}\u00a77 (max. {2})",
						ItemStackUtil.getItemName(new ItemStack(entry.getKey())), tr("\u00a7c{0}", "?"),
						entry.getValue()) + "\n");
			}
		}

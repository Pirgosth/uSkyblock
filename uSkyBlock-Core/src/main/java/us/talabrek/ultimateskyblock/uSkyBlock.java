...
import us.talabrek.ultimateskyblock.event.NetherPortalEvent;
...
public class uSkyBlock extends JavaPlugin implements uSkyBlockAPI, CommandManager.RequirementChecker {
	...
	private static final String[][] depends = new String[][] { new String[] { "Vault", "1.7.0", "optional" },
			new String[] { "WorldEdit", "7.0", "optionalIf", "FastAsyncWorldEdit" },
			new String[] { "WorldGuard", "7.0" }, new String[] { "FastAsyncWorldEdit", "1.13", "optional" },
	...
	// TODO: 28/06/2016 - R4zorax: These two should probably be moved to the proper
	// classes
	...
	public synchronized boolean isRequirementsMet(CommandSender sender, Command command, String... args) {
		if (maintenanceMode && !((command instanceof AdminCommand && args != null && args.length > 0
				&& args[0].equals("maintenance")) || command instanceof SetMaintenanceCommand)) {
			...
			for (String[] pluginReq : depends) {
				...
				if (pluginManager.isPluginEnabled(pluginReq[0])) {
					...
					if (VersionUtil.getVersion(desc.getVersion()).isLT(pluginReq[1])) {
						missingRequirements += tr(
								"\u00a7buSkyBlock\u00a7e depends on \u00a79{0}\u00a7e >= \u00a7av{1}\u00a7e but only \u00a7cv{2}\u00a7e was found!\n",
								pluginReq[0], pluginReq[1], desc.getVersion());
					}
				} else {
					missingRequirements += tr("\u00a7buSkyBlock\u00a7e depends on \u00a79{0}\u00a7e >= \u00a7av{1}",
							pluginReq[0], pluginReq[1]);
				}
			}
		}
	...
	public void registerEvents() {
		...
		manager.registerEvents(new NetherPortalEvent(), this);
	...
	public Location getSafeNetherHomeLocation(final PlayerInfo p) {
		Location home = LocationUtil.findNearestSafeLocation(p.getNetherHomeLocation(), null);
		if (home == null) {
			home = LocationUtil.findNearestSafeLocation(p.getIslandNetherLocation(), null);
		}
		return home;
	}
	...
	public boolean restartPlayerIsland(final Player player, final Location next, final String cSchem) {
		...
		if (getWorldManager().isSkyWorld(player.getWorld())) {
			// Clear first, since the player could log out and we NEED to make sure their
	...
	public synchronized boolean devSetPlayerIsland(final Player sender, final Location l, final String player) {
		...
		// Purge current islandinfo and partymembers if there's an active party at this
		// location (issue #948)
	...
	public boolean homeSet(final Player player) {
		if (!player.getWorld().getName().equalsIgnoreCase(getWorldManager().getWorld().getName())
				&& !player.getWorld().getName().equalsIgnoreCase(getWorldManager().getNetherWorld().getName())) {
			player.sendMessage(tr("\u00a74You must be in a skyblock workd to set your home!"));
			return true;
		}
		if (playerIsOnOwnIsland(player)) {
			...
			if (playerInfo != null && isSafeLocation(player.getLocation())) {
				if (player.getWorld().getName().equalsIgnoreCase(getWorldManager().getWorld().getName())) {
					...
					playerInfo.setNetherHomeLocation(player.getLocation());
					player.sendMessage(tr("\u00a7aYour nether skyblock home has been set to your current location."));
				}
				playerInfo.save();
			} else {
		...
		player.sendMessage(tr("\u00a74You must be on your island to set your skyblock home!"));
		return true;
	}
	...
	public boolean islandAtLocation(final Location loc) {
		return ((WorldGuardHandler.getIntersectingRegions(loc).size() > 0) || islandLogic.hasIsland(loc));
	...
	private void reloadConfigs() {
		...
		// levelLogic = AsyncWorldEditHandler.isAWE() ? new AweLevelLogic(this,
		// levelConfig) : new ChunkSnapshotLevelLogic(this, levelConfig);
	...
	public void execCommand(Player player, String command, boolean onlyInSky) {
		...
		command = command.replaceAll("\\{player\\}", Matcher.quoteReplacement(player.getName()))
				...
				.replaceAll("\\{playername\\}", Matcher.quoteReplacement(player.getDisplayName()))
				.replaceAll("\\{position\\}", Matcher.quoteReplacement(LocationUtil.asString(player.getLocation()))); // Figure
																														// out
																														// what
																														// this
																														// should
	...
	public String getVersionInfo(boolean checkEnabled) {
		...
		msg += pre("\u00a79  State: d={0}, r={1}, i={2}, p={3}, n={4}, awe={5}\n", Settings.island_distance,
				Settings.island_radius, islandLogic.getSize(), playerLogic.getSize(), Settings.nether_enabled,
				AsyncWorldEditHandler.isAWE());
		msg += pre("\u00a77Server: \u00a7e{0} {1}\n", getServer().getName(), getServer().getVersion());
		msg += pre("\u00a79  State: online={0}, bungee={1}\n", ServerUtil.isOnlineMode(), ServerUtil.isBungeeEnabled());
		msg += pre("\u00a77------------------------------\n");
	...
	private IslandScore adjustScore(IslandScore score, IslandInfo islandInfo) {
		...
		blockScore = blockScore * islandPerk.getScoreMultiply() * islandInfo.getScoreMultiplier()
				+ islandPerk.getScoreOffset() + islandInfo.getScoreOffset();
		return new IslandScore(blockScore, score.getTop());
	}

	public void calculateScoreAsync(final Player player, String islandName,
			final Callback<us.talabrek.ultimateskyblock.api.model.IslandScore> callback) {
		final IslandInfo islandInfo = getIslandInfo(islandName);
		getLevelLogic().calculateScoreAsync(islandInfo.getIslandLocation(), new Callback<IslandScore>() {
			...
			public void run() {
				...
				fireAsyncEvent(
						new uSkyBlockScoreChangedEvent(player, getInstance(), score, islandInfo.getIslandLocation()));
				callback.run();
			}
		});
	}
	...
	/**
	 * CAUTION! If anyone calls this with true, they MUST ensure it is later called
	 * with false, or the plugin will effectively be in a locked state.
	 *
	...
	public BukkitTask async(Runnable runnable, long delayMs) {
		return Bukkit.getScheduler().runTaskLaterAsynchronously(this, runnable, TimeUtil.millisAsTicks(delayMs));
	}

	public BukkitTask async(Runnable runnable, long delay, long every) {
		return Bukkit.getScheduler().runTaskTimerAsynchronously(this, runnable, TimeUtil.millisAsTicks(delay),
	...
	public BukkitTask sync(Runnable runnable, long delayMs) {
		return Bukkit.getScheduler().runTaskLater(this, runnable, TimeUtil.millisAsTicks(delayMs));
	}

	public BukkitTask sync(Runnable runnable, long delay, long every) {
		return Bukkit.getScheduler().runTaskTimer(this, runnable, TimeUtil.millisAsTicks(delay),
				TimeUtil.millisAsTicks(every));
	}

...
public class PlayerInfo implements Serializable, us.talabrek.ultimateskyblock.api.PlayerInfo {
	...
	private Location netherHomeLocation;
	...
	public PlayerInfo(String currentPlayerName, UUID playerUUID) {
		...
		if (!playerConfigFile.exists() && nameFile.exists()
				&& !currentPlayerName.equals(PlayerDB.UNKNOWN_PLAYER_NAME)) {
	...
	public void setNetherHomeLocation(final Location l) {
		this.netherHomeLocation = l != null ? l.clone() : null;
	}

	public Location getNetherHomeLocation() {
		return this.netherHomeLocation != null ? this.netherHomeLocation.clone() : null;
	}
	...
	public void completeChallenge(final String challenge, boolean silent) {
		...
		if (island != null) {
			island.sendMessageToOnlineMembers(
					tr("\u00a79{0}\u00a7f has completed the \u00a79{1}\u00a7f challenge!", getPlayerName(), challenge));
		}
	}
	...
	private void setupPlayer() {
		...
		pSection.set("netherHomeX", 0);
		pSection.set("netherHomeY", 0);
		pSection.set("netherHomeZ", 0);
		pSection.set("netherHomeYaw", 0);
		pSection.set("netherHomePitch", 0);
	...
	private PlayerInfo loadPlayer() {
		...
		try {
			...
			this.islandLocation = new Location(uSkyBlock.getInstance().getWorldManager().getWorld(),
					playerData.getInt("player.islandX"), playerData.getInt("player.islandY"),
					playerData.getInt("player.islandZ"));
			this.homeLocation = new Location(uSkyBlock.getInstance().getWorldManager().getWorld(),
					playerData.getInt("player.homeX") + 0.5, playerData.getInt("player.homeY") + 0.2,
					playerData.getInt("player.homeZ") + 0.5, (float) playerData.getDouble("player.homeYaw", 0.0),
					(float) playerData.getDouble("player.homePitch", 0.0));
			this.netherHomeLocation = new Location(uSkyBlock.getInstance().getWorldManager().getNetherWorld(),
					playerData.getInt("player.netherHomeX") + 0.5, playerData.getInt("player.netherHomeY") + 0.2,
					playerData.getInt("player.netherHomeZ") + 0.5,
					(float) playerData.getDouble("player.netherHomeYaw", 0.0),
					(float) playerData.getDouble("player.netherHomePitch", 0.0));
	...
	public void saveToFile() {
		...
		// TODO: 11/05/2015 - R4zorax: Instead of saving directly, schedule it for
		// later...
		log.entering(CN, "save", playerName);
		if (playerData == null) {
			LogUtil.log(Level.INFO,
					"Can't save player data! (" + playerName + ", " + uuid + ", " + playerConfigFile + ")");
			return;
		}
		FileConfiguration playerConfig = playerData;
		...
		Location netherHome = this.getNetherHomeLocation();
		if (netherHome != null) {
			playerConfig.set("player.netherHomeX", netherHome.getBlockX());
			playerConfig.set("player.netherHomeY", netherHome.getBlockY());
			playerConfig.set("player.netherHomeZ", netherHome.getBlockZ());
			playerConfig.set("player.netherHomeYaw", netherHome.getYaw());
			playerConfig.set("player.netherHomePitch", netherHome.getPitch());
		} else {
			playerConfig.set("player.netherHomeX", 0);
			playerConfig.set("player.netherHomeY", 0);
			playerConfig.set("player.netherHomeZ", 0);
			playerConfig.set("player.netherHomeYaw", 0);
			playerConfig.set("player.netherHomePitch", 0);
		}
		try {
	...
	public String toString() {
		...
		str += ChatColor.GRAY + "  - netherHome: " + ChatColor.DARK_AQUA + LocationUtil.asString(getNetherHomeLocation()) + "\n";
